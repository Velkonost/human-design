...
import com.myhumandesignhd.ui.start.ext.loginEmail
import com.myhumandesignhd.ui.start.ext.onSignupFinished
import com.myhumandesignhd.ui.start.ext.setupOldUsers1
import com.myhumandesignhd.ui.start.ext.setupOldUsers2
import com.myhumandesignhd.ui.start.ext.setupOldUsers3
import com.myhumandesignhd.ui.start.ext.setupOldUsers4
...
class StartFragment :
    BaseFragment<StartViewModel, com.myhumandesignhd.databinding.FragmentStartOldBinding>(
        R.layout.fragment_start_old,
...
//            )
    ...
    override fun onViewModelReady(viewModel: StartViewModel) {
        ...
        viewModel.skipOnboarding.observe(this) { skip ->
            if (skip == null) return@observe
            ...
            if (skip) {
                ...
                binding.signupView.inboxContainer.isVisible = false
        ...
        viewModel.loginFbLiveData.observe(this) { response ->
            ...
            if (response.data?.token?.isNullOrEmpty() == false) {
                App.preferences.authToken = response.data.token
//                App.preferences.uniqueUserId = response.data.userId
                onSignupFinished()
                EventBus.getDefault().post(ActivateAdaptyEvent())
            } else if (response.message.isEmpty().not()) {
                showError(getString(R.string.signup_user_not_logged_in))
            } else if (response.status.isEmpty().not()) {
...
//        binding.startBtn.translationY(requireContext().convertDpToPx(-44f), 500)
        ...
        inflatedNameContainer.nameTitle.text =
            App.resourcesProvider.getStringLocale(R.string.start_name_title)
        inflatedNameContainer.nameDesc.text =
            App.resourcesProvider.getStringLocale(R.string.start_name_desc)
        ...
        inflatedNameContainer.nameET.filters = arrayOf(filter)
    }
    ...
    private fun setupDateBirth() {
        ...
        inflatedNameContainer.nameTitle.text =
            App.resourcesProvider.getStringLocale(R.string.start_date_title)
...
//            inflatedNameContainer.placeET.requestFocus()
                ...
                android.os.Handler().postDelayed({
                    ...
                    android.os.Handler().postDelayed({
                        ...
                        if (!App.preferences.isPremiun && !isPaywallOpened) {
                            ...
                            android.os.Handler().postDelayed({
                                App.preferences.lastLoginPageId = -1
                                App.preferences.userNameFromStart =
                                    inflatedNameContainer.nameET.text.toString()
    ...
    fun showMessage(msg: String) {
        ...
        snackView.findViewById<TextView>(R.id.title).text =
            App.resourcesProvider.getStringLocale(R.string.snackbar_title_notification)
        snackView.findViewById<TextView>(R.id.desc).text = msg
        snackbar.setBackgroundTint(Color.parseColor("#F7C52B"))
    ...
    inner class Handler {
        ...
        fun onBackClicked(v: View) {
            ...
            when (currentStartPage) {
                StartPage.OLD_USERS_2 -> {
                    setupOldUsers1()
                }

                StartPage.OLD_USERS_3 -> {
                    setupOldUsers2()
                }

                StartPage.OLD_USERS_4 -> {
                    setupOldUsers3()
                }
...
//                    Navigator.startToBodygraph(this@StartFragment)
    ...
    override fun backPressed() {
        if (binding.placesView.isVisible) {
            binding.placesView.isVisible = false
        } else if (binding.signupView.inboxContainer.isVisible) {
            loginEmail()
            binding.signupView.inboxContainer.isVisible = false
        } else if (binding.signupView.emailContainer.isVisible) {
            setupSignup()
            binding.signupView.emailContainer.isVisible = false
        } else {
